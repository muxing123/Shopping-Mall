<template>
  <div class="shoppingcart">
    ...
    <div>
      <!--    用户存在时-->
      <div v-if="user && shopList.length > 0" class="user">
        <!--        全选与合计-->
        <div class="totalAll">
          <!--          全选框-->
          <div class="img" @click="checkall">
            <img
              src="../../icons/selsquare.png"
              alt="选中方框"
              v-if="checkAll"
            />
            <img src="../../icons/square.png" alt="方框" v-else />全选
          </div>
          <div class="total">
            合计:<Span>￥{{ sum }}</Span>
          </div>
            ...
            <div class="shopDetal">
              ...
              <div class="img" @click="checkone(item, item.cid)">
                <img
                  ...
                  v-if="item.check"
                />
                <img src="../../icons/square.png" alt="方框" v-else />
              </div>
              <!--          商品图片-->
              <div class="shopImg">
              ...
              <div class="info">
                ...
                <div class="priceNum">
                  <div class="present_price">
                    ￥{{ (item.present_price * item.count).toFixed(2) }}
                  </div>
                  <div class="count">
                    <div class="jian" @click="reduce(item)">
                      <img src="../../icons/jianhao.png" alt="减" />
                    </div>
                    <div class="text">{{ item.count }}</div>
                    <div class="plus" @click="plus(item)">
                      <img src="../../icons/plus.png" alt="加" />
                    </div>
                  </div>
                </div>
              </div>
      ...
      <!--      用户存在但购物车没有商品-->
      <div v-else-if="user && shopList.length === 0" class="userNo">
        <!--    图片-->
        <div class="img">
          <img src="../../icons/shop.png" alt="购物车" />
        </div>
        <!--    文字-->
        <div class="text">
          您的购物车还是空空的哦~~
        ...
        <!--    去购物-->
        <div class="button">
          <van-button round type="default" color="black" plain to="shoppingmall"
            >去购物</van-button
          >
        </div>
      </div>
      <!--    用户不存在时-->
      <div v-else class="userNo">
        <!--    图片-->
        <div class="img">
          <img src="../../icons/shop.png" alt="购物车" />
        </div>
...
export default {
  ...
  //   if (to.path === "/loginregistration") {
  //     next();
  //   } else {
  //     user ? next() : next("/loginregistration");
  //   }
  // },
  methods: {
    //加
    ...
    //  结算
    Settlement() {
      this.shopSelect = this.shopList.filter(item => {
        return item.check === true;
      });
      localStorage.setItem(
        this.user + "shoplist",
        JSON.stringify(this.shopSelect)
      );
      this.$router.push("/order");
      // this.$router.push({
      //   name: "order",
      //   query: { data: this.shopSelect }
      // });
    }
  },
  mounted() {
